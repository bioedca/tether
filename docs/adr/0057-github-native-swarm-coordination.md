...
(`reaper.py` and `triage.py`), `agent:round-1` / `agent:review-capped`, and `agent:needs-advance` (`triage.py`; ADR-0062 and
#394 — a clean review on an unfinished lane, authorising exactly one advance session).
