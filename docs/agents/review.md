...
- **On agent-layer paths that rule inverts: a sub-floor finding is dropped, not tracked.** The paths
  ...
  re-enters this same lane and produces more sub-floor findings. Measured: **sixteen** agent-layer
  issues came from that loop in ten days, thirteen of them in the last five.
