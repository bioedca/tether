...
Only the cross-correlation has a tMAVEN counterpart at all, and Tether's is a deliberate
divergence rather than a port: tMAVEN's `selection.py` uses a raw, gradient-based, lag-zero-only
heuristic. Most, but not all, carry the rendering-default label in the constant's own comment in
