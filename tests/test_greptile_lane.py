...
def test_the_config_is_valid_json_with_no_unknown_top_level_keys() -> None:
    ...
    """
    known = {
        "$schema", "skipReview", "triggerOnDrafts", "triggerOnUpdates", "strictness",
        "commentTypes", "instructions", "labels", "disabledLabels", "includeBranches",
        "excludeBranches", "includeAuthors", "excludeAuthors", "includeKeywords",
        "ignoreKeywords", "fileChangeLimit", "ignorePatterns", "context", "customContext",
        "patternRepositories", "shouldUpdateDescription", "updateSummaryOnly", "fixWithAI",
        "hideFooter", "includeIssuesTable", "includeConfidenceScore", "includeSequenceDiagram",
        "statusCheck", "statusCommentsEnabled", "summarySection", "issuesTableSection",
        "confidenceScoreSection", "sequenceDiagramSection",
    }
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
