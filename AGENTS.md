...
- **The issue's own `Execution autonomy` is part of that precondition**, and no label overrides it.
  Anything but `agent-can-do-alone` (or the older `agent can complete alone`, still accepted) —
  including a body that declares nothing, that declares it and then also names a maintainer
  decision, or that declares it twice and disagrees — exits `3` naming the value. Separators do not
  matter and position does not either: one restrictive declaration governs however many admitting
  ones sit beside it. A label is applied *to* an issue; this is the issue's own statement about
