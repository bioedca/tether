...
- **The issue's own `Execution autonomy` is part of that precondition**, and no label overrides it.
  Anything but `agent-can-do-alone` — including a body that declares nothing, or that declares it
  and then also names a maintainer decision — exits `3` naming the value. A label is applied *to* an
