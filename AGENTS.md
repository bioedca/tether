...
- **The issue's own `Execution autonomy` is part of that precondition**, and no label overrides it.
  Anything but `agent-can-do-alone` — including a body that declares nothing, or that declares it and
  then also names a maintainer decision — exits `3` naming the value. A label is applied *to* an
  issue; this is the issue's statement about what finishing it requires.
...
- **The agent layer is feature-complete** (ADR-0064). `.agents/` and this file accept bug fixes and
  safety fixes only; a capability change needs a maintainer-opened issue and may not originate in a
  review finding.
