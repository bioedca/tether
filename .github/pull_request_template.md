...
- [ ] **Review complete** (`AGENTS.md` §Review) — the diff went green on a draft first; at least one external provider reviewed the **final head** and its verdict is quoted above with the `commit_id` it read; and **CodeRabbit returned no actionable comments at that head**, asked with the **full-review** command. Neither silence nor a green `CodeRabbit` status check is the gate — both are also what a request that reviewed *nothing* leaves behind. A provider that could not act is recorded above with the reason, and **quota is *did not review*, never a pass**. Serious findings fixed; the rest deferred to one follow-up issue, or dropped without one if this is an agent-layer path (ADR-0064). Every conversation resolved.
